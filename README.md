...
Распространенные статические фабричные методы:
...
Это подходит для классов, который имеет только статические методы и статические поля (утильные классы).

Например:

- Группировка связанных методов над примитивами или массивами (`java.util.Arrays`)
- Группировка статических методов и фабрики (`java.util.Collections`)
...
Не стоит использовать класс синглтон или утильный(статический), если они зависят от ресурсов(объектов), поведение
которых влияют на них. И не стоит предоставлять создание ресурсов этим классам.
Лучше предавать ресурсы или их фабрики для создания, конструктору (или статической фабрике, или строителю). Этот
...
## 2.9 Предпочитайте try-with-resources использованию try-finally (Item 9)

Предпочитайте `try-with-resources` применению `try-finally` при работе с ресурсами, которые должны быть закрыты.
...
Оператор `try-with-resources` облегчает написание корректного кода с использованием ресурсов, которые должны быть закрыты,
...
статические методы `compare` классов оберток примитивных типов(`Integer.compare` и др.) или методы конструирования
...
статическую фабрику ([Item 1](#21-рассмотрите-применение-статических-фабричных-методов-вместо-конструкторов-item-1)).
...
- Объявить как `final`
- Сделать все конструкторы закрытыми или доступные в пределах пакета, а вместо них использовать статические фабрики.
...
* Если класс внутри другого класса должен быть виден за пределами метода или он слишком длинный, для размещения в
  границах метода, используйте вложенный класс(static или nonstatic).
    ...
    * В остальных случаях статический(static member class)
...
| статические фабрики                                          | см. [Item 1](#21-рассмотрите-применение-статических-фабричных-методов-вместо-конструкторов-item-1)              | valueOf, of, getInstance, newInstance, getType, newType   |
